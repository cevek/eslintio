                ...
                if (hasProblem) {
                    context.report({
                        ...
                        fix: fixer => {
                            ...
                            return sortedGroups.flatMap(({item, props}, index) => {
                                const newProps = [];
                                const oldProps = [];
                                const originalGroup = groups[index];
                                if (originalGroup.item) {
                                    oldProps.push(originalGroup.item);
                                }
                                oldProps.push(...originalGroup.props);
                                if (item) {
                                    newProps.push(sourceCode.getText(item));
                                }
                                newProps.push(...props.map(prop => sourceCode.getText(prop)));
                                return oldProps.map((oldProp, index) => fixer.replaceText(oldProp, newProps[index]));
                            });
                        },
                    });
                }
